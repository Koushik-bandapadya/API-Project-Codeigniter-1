...
//        	}
		...
		//$this->db->join('plant_detail','plant_detail.id=order_table.plant', 'left');
	...
	public  function getTodayForecast($date, $orderType){
		...
		$this->db->from('order_table');
		$this->db->where('order_table.order_type',$orderType);
		$this->db->where('order_table.delevary_date', $date);
		$rslt = $this->db->get();
		$result = $rslt->result_array();
		$que=$this->db->last_query();
		return $result;
	...
	public function getProductBytxid($txid,$orderType){
		...
		$this->db->from('order_table');
		$this->db->where('order_table.order_type',$orderType);
		$this->db->where('order_table.txid', $txid);
		$rslt = $this->db->get();
		$result = $rslt->result_array();
		$que=$this->db->last_query();
		return $result;
...
//
//SELECT table1.*
//FROM order_table as table1
//LEFT JOIN order_table as table2 ON table2.client_id=table1.client_id
//WHERE table2.product_id != table1.product_id

/*SELECT table1.*, product_details.p_name, product_details.p_type
FROM order_table as table1
LEFT JOIN order_table as table2 ON table2.product_id != table1.product_id
