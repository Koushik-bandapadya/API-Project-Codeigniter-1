...
class Employee_wise_order_model extends CI_Model{
	...
	public function getEmployeeWiseOrderDetails($designation, $start_date, $end_date, $order_type){
		...
		}else
		{
			...
			foreach ($parentList as $parentItem){
				...
				$parentItem['orders']=$this->getClientOrderSum($parentItem['coded_employeeId'],$order_type,$start_date,$end_date);
	...
	private function getClientOrderSum($id,$orderType,$start_date,$end_date){
		...
		$clientOrderList = $this->getEmployeeClientOrderDetails($id,$orderType,$start_date,$end_date);
	...
	private function getEmployeeClientOrderDetails($id,$orderType,$start_date,$end_date){
		$this->db->select('*');
